...
class SfdcGenerateExcelController {
  static void main(String[] args){
    ...
    yaml.resources.collect{ createResourceDocument(inputBaseDir, outputDir, it) }
  }
  ...
  private static void createResourceDocument(String inputBaseDir, String outputDir, def resourceYaml){
    SfdcCustomObject object = createCustomObject(inputBaseDir, resourceYaml.resource?.object)
  ...
  private static SfdcCustomObject createCustomObject(String inputBaseDir, String param){
  ...
  private static SfdcObject createObject(String inputBaseDir, String param){
    assert param : "'object' parameter is not set"
    def xmlFile = getPathIfExists(inputBaseDir + param)
    def fileName = xmlFile.getName().find('[^/]*$')
    return new SfdcObject(fileName, xmlFile.text)
  }
