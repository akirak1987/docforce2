...
class SfdcWorkflowRule {
  ...
  public SfdcWorkflowRule(NodeChild xml){
    this.displayLabel = xml.fullName
    this.evaluateTerm = SfdcCriteria.getCriteria(xml)
    this.triggerType = convertTriggerType(xml.triggerType)
    this.actionList = xml.actions.collect{ new Action(it.type, it.name) }
  }
